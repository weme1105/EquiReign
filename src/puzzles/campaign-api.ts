...
export async function downloadCampaignBatch(startLevel: number): Promise<CachedCampaignBatch | null> {
  ...
  const maxEndLevel = startLevel + 99;
  if (payload.startLevel !== startLevel || payload.endLevel < startLevel || payload.endLevel > maxEndLevel || !Array.isArray(payload.puzzles)) {

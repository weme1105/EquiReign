...
export class RegionPuzzleGenerator implements PuzzleGenerator {
  generate(size: number, seed = Date.now()): GeneratedPuzzle {
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      ...
      if (!columns) continue;
      const regionMap = growRegions(size, columns, random);
...
/**
 * Grow connected regions around a known legal solution.
 * The seeded queens already witness one legal solution. Assigning additional
 * non-queen cells to existing regions cannot invalidate that witness, so a
 * solver call during every growth step is redundant. Uniqueness is checked
 * once, after the complete region map has been built.
 */
function growRegions(size: number, queens: readonly number[], random: () => number): number[] | null {
  ...
  const addFrontier = (row: number, column: number) => {
    for (const [dr, dc] of directions) {
      ...
      if (r >= 0 && c >= 0 && r < size && c < size && regions[r * size + c] === -1) frontier.add(r * size + c);
    }
  };
  ...
  while (regions.includes(-1)) {
    ...
    for (const index of frontier) {
      ...
      for (const [dr, dc] of directions) {
        const r = row + dr; const c = column + dc;
        if (r >= 0 && c >= 0 && r < size && c < size && regions[r * size + c]! >= 0) adjacent.add(regions[r * size + c]!);
      }
      for (const region of adjacent) {
        options.push({ index, region, score: sizes[region]! * 2 + Math.abs(row - region) + Math.abs(column - queens[region]!) + random() });
      }
    }
    options.sort((a, b) => a.score - b.score);
    const accepted = options[0];
    if (!accepted) return null;
    regions[accepted.index] = accepted.region;

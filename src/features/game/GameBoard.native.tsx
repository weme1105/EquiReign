...
export function GameBoard({ session, onPress, onDoublePress, onDragToggleExcluded, dualColorCellIndexes = [], showDualRegions = false }: Props) {
  ...
  const [mistakeFlash, setMistakeFlash] = useState(true);
  useEffect(() => { if (!session.mistakeErrorKeys.length) { setMistakeFlash(true); return; } const timer = setInterval(() => setMistakeFlash((current) => !current), 150); return () => clearInterval(timer); }, [session.mistakeErrorKeys]);

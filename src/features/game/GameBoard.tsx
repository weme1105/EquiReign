...
export function GameBoard({ session, onPress, onDoublePress, dualColorCellIndexes = [], showDualRegions = false }: Props) {
  ...
  const startDrag = (index: number, x: number, y: number) => { clearPendingTap(); if (isProtected(index)) return; const state = session.boardState.cells[index]!; drag.current = { lastIndex: index, startIndex: index, startX: x, startY: y, mode: state === 'excluded' ? 'erase-x' : 'fill-x', dragging: false, visited: new Set<number>() }; };

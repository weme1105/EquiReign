...
interface TapMemory {
  ...
  readonly originalState: CellState;
}

interface DragMemory {
  readonly startIndex: number;
  lastIndex: number;
  readonly startX: number;
  readonly startY: number;
  readonly mode: 'fill-x' | 'erase-x';
  dragging: boolean;
...
export function GameBoard({ session, onPress, onLongPress, dualColorCellIndexes = [], showDualRegions = false }: Props) {
  ...
  const isProtected = (index: number): boolean => {
    const row = Math.floor(index / session.puzzle.size); const column = index % session.puzzle.size;
    return isGivenQueen(session, { row, column })
      || session.lostCellIndexes.includes(index)
      || (session.frozenCellIndexes.includes(index) && !session.revealedFrozenCellIndexes.includes(index));
  };

  const indexFromPoint = (x: number, y: number): number | null => {
    const localX = x - BOARD_BORDER_WIDTH; const localY = y - BOARD_BORDER_WIDTH;
  ...
  const invokeForIndex = (index: number, action: 'press' | 'toggle-x') => {
    ...
    if (action === 'press') onPress(row, column); else onLongPress(row, column);
  };

  const handleTap = (index: number) => {
    if (isProtected(index)) { lastTap.current = null; return; }
    const now = Date.now(); const state = session.boardState.cells[index]!;
    const previous = lastTap.current;
    if (previous && previous.index === index && now - previous.atMs <= DOUBLE_TAP_WINDOW_MS) {
      // Second tap is interpreted from the state before the first tap:
      // empty -> X -> queen, X -> empty -> queen, queen -> empty -> X.
    ...
    // Single tap: empty -> X, X -> empty, queen -> empty.
    if (state === 'queen') invokeForIndex(index, 'press'); else invokeForIndex(index, 'toggle-x');
  ...
  const unitHasNoQueenAndAllX = (index: number): boolean => {
    const size = session.puzzle.size; const row = Math.floor(index / size); const column = index % size;
  ...
  return <View
    ...
    onResponderGrant={(event) => {
      const index = indexFromPoint(event.nativeEvent.locationX, event.nativeEvent.locationY);
      if (index === null || isProtected(index)) { drag.current = null; return; }
      const state = session.boardState.cells[index]!;
      drag.current = {
        startIndex: index, lastIndex: index,
        startX: event.nativeEvent.locationX, startY: event.nativeEvent.locationY,
        mode: state === 'excluded' ? 'erase-x' : 'fill-x', dragging: false, visited: new Set<number>(),
      };
    }}
    onResponderMove={(event) => {
      const memory = drag.current; if (!memory) return;
      const index = indexFromPoint(event.nativeEvent.locationX, event.nativeEvent.locationY); if (index === null) return;
      if (!memory.dragging) {
        const dx = event.nativeEvent.locationX - memory.startX; const dy = event.nativeEvent.locationY - memory.startY;
        if (Math.hypot(dx, dy) < DRAG_THRESHOLD_PX) return;
        memory.dragging = true; lastTap.current = null; applyDragIndex(memory, memory.startIndex);
      }
      applyDragLine(memory, memory.lastIndex, index); memory.lastIndex = index;
    }}
    onResponderRelease={() => {
      const memory = drag.current; drag.current = null; if (!memory) return;
      if (!memory.dragging) handleTap(memory.startIndex);
    }}
    onResponderTerminate={() => { drag.current = null; lastTap.current = null; }}
    ...
    {session.boardState.cells.map((state, index) => {
      ...
      const key = positionKey(position); const given = isGivenQueen(session, position);
      // Beginner/intermediate assistance marks only the most recently changed cell.
      // It reports immediate local contradictions, never full-board solver feasibility.
      const error = lastInteractedIndex === index && (conflicts.has(key) || unitHasNoQueenAndAllX(index));
      ...
      return <Pressable key={key} accessibilityRole="button" accessibilityState={{ disabled: given || lost || frozen }}
        ...
        disabled={given || lost || frozen} onPress={() => {}}
...
const styles = StyleSheet.create({
  ...
  error: { backgroundColor: '#d86470', borderColor: '#ffabb3', borderWidth: 3 }, hinted: { borderColor: '#fff29d', borderWidth: 4 }, queen: { color: '#17142a', fontSize: 29, lineHeight: 35 },

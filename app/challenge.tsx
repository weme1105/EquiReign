...
export default function ChallengeScreen() {
  ...
  return <SafeAreaView style={styles.screen}><ScrollView contentContainerStyle={styles.content}><Pressable onPress={() => router.back()}><Text style={styles.back}>‹ 返回</Text></Pressable><Text style={styles.title}>難度挑戰</Text>
    ...
    <Text style={styles.label}>尺寸</Text><View style={styles.wrap}>{(['random', ...CHALLENGE_BOARD_SIZE_ORDER] as const).map((item) => <Pressable key={item} onPress={() => setSize(item)} style={[styles.choice, size === item && styles.selected]}><Text style={styles.choiceText}>{item === 'random' ? '隨機' : `${item}×${item}`}</Text></Pressable>)}</View>
    <Text style={styles.note}>目前正式挑戰池：6×6–12×12；13×13–20×20 已納入 runtime，待大型題庫完成後開放。</Text>
    {selectedCount !== null && <Text style={styles.count}>此組合已成功 {selectedCount} 次</Text>}
    <Pressable onPress={start} style={styles.start} testID="challenge-start"><Text style={styles.startText}>開始挑戰</Text></Pressable>
  </ScrollView></SafeAreaView>;
}
const styles = StyleSheet.create({ screen: { backgroundColor: '#17142a', flex: 1 }, content: { alignSelf: 'center', maxWidth: 560, padding: 28, width: '100%' }, locked: { alignSelf: 'center', flex: 1, justifyContent: 'center', maxWidth: 480, padding: 28, width: '100%' }, lockedText: { color: '#aaa4bc', marginTop: 12 }, back: { color: '#aaa4bc', fontSize: 16, marginBottom: 28 }, title: { color: '#fffaf1', fontSize: 36, fontWeight: '900' }, label: { color: '#ddd6e8', fontSize: 14, fontWeight: '800', marginBottom: 10, marginTop: 26 }, wrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, choice: { borderColor: '#514a70', borderRadius: 11, borderWidth: 1, paddingHorizontal: 15, paddingVertical: 11 }, selected: { backgroundColor: '#806d36', borderColor: '#d6b870' }, choiceText: { color: '#fffaf1', fontWeight: '800' }, note: { color: '#777087', fontSize: 12, lineHeight: 18, marginTop: 16 }, count: { color: '#f0d58e', fontWeight: '800', marginTop: 18 }, start: { alignItems: 'center', backgroundColor: '#d6b870', borderRadius: 14, marginTop: 34, padding: 15 }, startText: { color: '#17142a', fontSize: 17, fontWeight: '900' } });
